...
class App:
    ...
    def setup(self):
        ...
        # ===== CONFIG =====
        MIN_BOX_W = 80   # minimum width per item
        MIN_BOX_H = 60   # minimum height per item (usually it is -PADDING)
        TOP_BAR = 12 
        PADDING = 4
        self.TEXT_PADDING = 2
    ...
    def run(self):
        ...
        while True:
            ...
            # SELECT (dpad center OR action A)
            if (
                cos.input.was_pressed_cap("dpad", "CENTER") or
                cos.input.was_pressed_cap('action','A')
            ):
