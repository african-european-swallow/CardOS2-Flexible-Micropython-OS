...
"""
todo:
in input/hw: allow app to use certain i2c themselse -- possibly done
make the kernal less hardcoded, not direct LOADING txt etc.
update system bar to allow for things like notifcations
system color list for unified os look
"""
...
def run(next_app):
    ...
    while True:
        ...
        # RUN TASK
        if cos.intent.is_intent(app_intent, cos.intent.INTENT_TASK):
