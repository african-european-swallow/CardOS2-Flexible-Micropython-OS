...
class App:
    def __init__(self, cos):
        ...
        # single output system (ONLY STATE OWNER)
    ...
    def run(self):
        ...
        while True:
            ...
            if cos.input.was_pressed_cap("action", "Y"):
                ...
                if self.block_mode and self.output.live_input == '':
                    curr = '    '
                else:
            ...
            if cos.input.was_pressed_cap("keyboard", "ENTER") or cos.input.was_pressed_cap("action", "B"):
                ...
                if self.block_mode and not consumed:
                    ...
                    # EXIT BLOCK ON EMPTY ENTER (your requirement)
                    if line == "":
...
class PrintOutput:
    def __init__(self, cos):
        ...
        self.screen_w = cos.gfx.screen_x
        self.screen_h = cos.gfx.screen_y
    ...
    def draw(self, x=0, y=0, color=(255, 255, 255)):
        ...
        while i < len(text):
            ...
            self.cos.gfx.text(
                chunk,
                x,
                y + y_offset + row * (self.char_h + self.spacing),
                (0, 255, 0)
            )
            i += self.chars_per_line
            row += 1
